...
import { ShoppingBagIcon, ShoppingCartIcon, GiftIcon, PlayIcon } from '@heroicons/react/24/outline';
        ...
        <div className='w-full'>
            <footer>
                ...
                <div className='flex justify-around w-full bg-blue-600 text-white py-2'>
                    <div>
                        <input dir="ltr" type="email" placeholder='Nhập email để nhận thông tin khuyến mãi'
                            className='rounded-s-lg px-4 py-2 bg-white text-black w-90 text-base' />
                        <button dir="rtl" className='rounded-s-lg px-4 py-2 bg-[#003cbf] text-white font-thin'>ĐĂNG KÝ</button>
                    </div>
                    <div className="flex items-center gap-3">
                        <p className='font-medium'>Kết nối với chúng tôi:</p>
                        <a href=""><ChatBubbleLeftRightIcon className="w-8 h-8 text-white hover:opacity-90" /></a> {/* Zalo */}
                        <a href=""><GlobeAltIcon className="w-8 h-8 text-white hover:opacity-90" /></a> {/* Google */}
                        <a href=""><PlayCircleIcon className="w-8 h-8 text-white hover:opacity-90" /></a> {/* YouTube */}
                        <a href=""><PhoneIcon className="w-8 h-8 text-white hover:opacity-90" /> </a>{/* Facebook placeholder */}
                    </div>
                </div>
                {/* Mid Footer */}
                <div className="bg-white text-dark py-10 text-sm">
                    <div className="mx-auto max-w-7xl px-4">
                        <div className="grid grid-cols-1 md:grid-cols-5 gap-8">
                            ...
                            {/* Chính sách (Policies) */}
                            <div>
                                <h3 className="text-lg font-semibold mb-4 text-[#003cbf]">CHÍNH SÁCH</h3>
                                <ul className="space-y-2">
                                    {policies.map((policy, index) => (
                                        <li key={index}>
                                            <a href="" className="hover:text-blue-700">{policy}</a>
                                        </li>
                                    ))}
                                </ul>
                            </div>
                            ...
                            {/* Hướng dẫn (Guides) */}
                            <div>
                                <h3 className="text-lg font-semibold mb-4 text-[#003cbf]">HƯỚNG DẪN</h3>
                                <ul className="space-y-2">
                                    {guides.map((guide, index) => (
                                        <li key={index}>
                                            <a href="" className="hover:text-blue-700">{guide}</a>
                                        </li>
                                    ))}
                                </ul>
                            </div>
                            ...
                            <div>
                                <h3 className="text-lg font-semibold mb-4 text-[#003cbf]">DANH MỤC</h3>
                                <ul className="space-y-2 mb-6">
                                    {categories.map((category, index) => (
                                        <li key={index}>
                                            <a href="" className="hover:text-blue-700">{category}</a>
                                        </li>
                                    ))}
                                </ul>
                            </div>
                            ...
                            <div>
                                ...
                                <p className="text-base font-semibold mb-2 text-[#003cbf]">190006750</p>
                                <p className="text-sm mb-2">GÓP Ý & KHIẾU NẠI (08:30 – 20:30)</p>
                                <p className="text-base font-semibold mb-2 text-[#003cbf]">190006750</p>
                                <p className="text-sm mb-4">Tất cả các ngày trong tuần (Trừ tết Âm Lịch)</p>

                                <h3 className="text-lg font-semibold mb-4 text-[#003cbf]">LIÊN KẾT SÀN</h3>
