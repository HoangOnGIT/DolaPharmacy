...
const Menu = () => {
  ...
  return (
    <div className="relative">
      <ul className="flex flex-wrap items-center gap-4 text-sm font-semibold relative z-10 my-3">
          ...
          return (
            ...
            >
              <Link to={item.path} className="flex items-center text-base">
                {item.title}
                {hasSubMenu && <ChevronDownIcon className="w-4 h-4 text-gray-500 ml-1 text-white" />}
              </Link>
                  ...
                  <ul className="space-y-2 text-gray-700">
                    {hasSubMenu.map((sub, i) => (
                      <li key={i}>
                        <Link
                          to={sub.path}
                          className="block hover:text-green-600 transition-colors duration-200 py-1"
                        >
                          {sub.title}
                        </Link>
                      </li>
